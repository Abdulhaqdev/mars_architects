...
export default function Hero() {
  ...
  // Update currentHash when the hash changes
  useEffect(() => {
    const updateHash = () => {
      ...
      <nav className="flex justify-between container h-24 items-center w-full md:px-2 py-4 md:py-6 z-20">
        ...
        <div>
          <Image
            ...
            height={110}
            width={60}
            className="absolute top-3 md:top-8 w-10 h-[70px] md:w-[60px] md:h-[110px]"
            priority
          />
        </div>
        {/* Social Icons */}
      ...
      <div className="h-[calc(100vh-1px)] container md:h-[calc(600px-400px)]">
        ...
        <div className="absolute md:right-0 h-3/4 max-w-6xl z-0">
          <Image
            ...
            className="object-cover h-[52%] md:h-[600px] w-full md:w-auto max-w-full"
            width={1100}
            height={1200}
            priority
          />
        ...
        <div className="relative z-10 h-[calc(720px-1px)] md:h-[calc(650px-1px)] flex flex-col gap-5 justify-center align-bottom max-w-5xl">
          ...
          <p className="text-[18px] font-[350] md:font-normal md:text-lg max-w-md">
            Мы создаем архитектуру и интерьеры, которые вдохновляют, делают жизнь
            удобнее
          </p>
          <div className="flex mt-6 justify-start flex-col-reverse md:flex-row">
            <div className="flex items-center gap-4 mt-6">
              ...
              <button
                className="w-8 h-8 rounded-full bg-[#c2000a] flex items-center justify-center"
                aria-label="Next"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
